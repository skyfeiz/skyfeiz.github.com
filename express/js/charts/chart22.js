...
(function () {
    ...
    var dataArr = [
        {name: 'Galaxy S7', value1: 220, value2: 200},
        {name: '魅族 Note6', value1: 300, value2: 320},
        {name: 'iPhone 8', value1: 200, value2: 240},
        {name: '荣耀 畅玩6X', value1: 330, value2: 150},
        {name: '红米 Note5A', value1: 290, value2: 200}
    ];
    var columnW = 35, columnH = 450;

    var Chart22 = function (scene, resourcesMap) {
        this.scene = scene;
        this.resourcesMap = resourcesMap;
        this.init();
    }

    var p = Chart22.prototype;

    p.init = function () {
        this.container = new THREE.Object3D();
        this.container.position.x = positionInfo[1].x;
        this.container.position.y = positionInfo[1].y;
    ...
    p.setData = function (value) {
        ...
        value.map(function (data) {
            cur.maxValue = Math.max(data.value1, data.value2, cur.maxValue);
            cur.minValue = Math.min(data.value1, data.value2, cur.minValue);
        });
    ...
    p.startRender = function () {
        ...
        this.nameItemArr.map(function (item, index) {
    ...
    p.createTitle = function () {
        ...
        this.titleContainer.userData['z'] = 200;
        ...
        obj['context'].fillText('仓库量概览', 150, 32);
        obj['material'].map.needsUpdate = true;
        title.position.y = 10;
        this.titleContainer.add(title);
        // var obj = this.createTxtMaterial(300, 64, 0, 36, 'left', true, '#00aeff');
        // var title = new THREE.Mesh(new THREE.PlaneBufferGeometry(300, 64), obj['material']);
        // obj['context'].clearRect(0, 0, 300, 64);
        ...
        obj['context'].clearRect(0, 0, 300, 64);
        obj['context'].fillText('STOCK OVERVIEW', 150, 32);
        obj['material'].map.needsUpdate = true;
    ...
    p.createAxisNumAndTick = function (array) {
        ...
        for (var i = 0; i <= splitNum; i++) {
            ...
            var obj = this.createTxtMaterial(128, 32, 0, 26, 'right', false, '#00aeff');
    ...
    p.createColumns = function () {
        ...
        this.datas.map(function (item, index) {
            ...
            column1Container.position.x = (columnW + 90) * index - 200;
            cur.container.add(column1Container);
            cur.columnArr.push(column1Container);

            var column = cur.createColumnItem(0x00fff6);
            column.scale.y = item['value1'] / cur.maxValue;
            column1Container.add(column);
            column.userData['clickable'] = true;
            cur.columnItems.push(column);

            // cur.createColumnNum(item, 'value', '未处理', column1Container);

            // var lightColumn = cur.createLightColumnItem(index);
            // lightColumn.scale.y = item['value1'] / cur.maxValue;
            ...
            column2Container.position.x = (columnW + 90) * index - 150;
            cur.container.add(column2Container);
            cur.columnArr.push(column2Container);

            var column = cur.createColumnItem(0x0061e6);
            column.scale.y = item['value2'] / cur.maxValue;
            column2Container.add(column);
            column.userData['clickable'] = false;
            cur.columnItems.push(column);

            // var lightColumn = cur.createLightColumnItem(index);
            // lightColumn.scale.y = item['value2'] / cur.maxValue;
            ...
            nameItem.position.x = (columnW + 80) * index - 200;
            cur.container.add(nameItem);
            cur.nameItemArr.push(nameItem);
    ...
    p.createColumnNum = function (item, key, txt, container) {
        ...
        //     num.position.x = -columnW / 2 - 10;
        //     num.position.y = item[key] / cur.maxValue * columnH + 20;
        //     if (key == 'left') {
    ...
    p.createLegend = function () {
        var legend = this.createLegendItem('当前仓储量', 0x00fff6);
        legend.position.set(450, 0, 60);
        this.container.add(legend);

        legend = this.createLegendItem('上一期仓储量', 0x0061e6);
        legend.position.set(450, 0, 0);
        this.container.add(legend);
    }

    p.createLegendItem = function (text, color) {
        var container = new THREE.Object3D();
        ...
        var obj = this.createTxtMaterial(200, 64, 1, 24, 'center', false, '#00aeff');
        var nameItem = new THREE.Mesh(new THREE.PlaneBufferGeometry(200, 64), obj['material']);
        obj['context'].clearRect(0, 0, 200, 64);
        obj['context'].fillText(text, 100, 44);
        obj['material'].map.needsUpdate = true;
        nameItem.position.set(100, 0, -5);
        // nameItem.rotation.x = -Math.PI / 2;
    ...
    p.createName = function (name) {
        ...
        container.userData['z'] = columnW * 2.5;
        ...
        var obj = this.createTxtMaterial(200, 64, 0, 24, 'center', false, '#00aeff');
        var nameItem = new THREE.Mesh(new THREE.PlaneBufferGeometry(200, 64), obj['material']);
        obj['context'].clearRect(0, 0, 200, 64);
